...
use crate::{defined_constants::MANIFEST_FILE_NAME, error::{ChurError, ChurResult}};
...
impl Manifest {
    pub(crate) fn load() -> ChurResult<Self> {
        let mut manifest_file = fs::OpenOptions::new().read(true).open(MANIFEST_FILE_NAME.as_path())?;
        let mut buf = String::new();

        manifest_file.read_to_string(&mut buf)?;

        ron::from_str::<Self>(&buf)
            .map_err(ChurError::from)
    }

    pub(crate) fn save(&self) -> ChurResult<()> {
        fs::create_dir_all(MANIFEST_FILE_NAME.parent().unwrap())?;
        let manifest_file = fs::OpenOptions::new().create(true).write(true).open(MANIFEST_FILE_NAME.as_path())?;
